...
- Make sure you have copied the `bin` folder in your Godot project, which contains `libprologot.so` for Linux, `libprologot.dylib` for macOS, or `libprologot.dll` for Windows. Also check that the corresponding SWI-Prolog library (`libswipl.so`, `libswipl.dylib`, or `libswipl.dll`) is present alongside the Prologot library file.
- Verify that the `prologot.gdextension` file in your project root points to the correct library path (`libprologot.so`, `libprologot.dylib`, or `libprologot.dll`, depending on your platform).
...
```gdscript
   prolog.initialize({
       ...
       "optimised": true       # Enable optimizations
```
